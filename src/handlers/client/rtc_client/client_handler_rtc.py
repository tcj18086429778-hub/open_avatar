...
_video_codec = os.getenv("OAC_VIDEO_CODEC", "vp8").strip().lower()
if _video_codec in {"h264", "h.264", "avc"}:
    _prioritize_h264()
    _configure_h264_hardware_encoding()
else:
    logger.info("Using default video codec order; set OAC_VIDEO_CODEC=h264 to prefer H.264")
